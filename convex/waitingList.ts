...
export const expireOffer = internalMutation({
  ...
  handler: async (ctx, { waitingListId, eventId }) => {
    ...
    await ctx.runMutation(processQueue, { eventId }); // await processQueue(ctx, { eventId });
  },
});
...
export const cleanupExpiredOffers = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    for (const [eventId, offers] of Object.entries(grouped)) {
      ...
      await ctx.runMutation(processQueue, { eventId: eventId as Id<"events"> }); // await processQueue(ctx, { eventId: eventId as Id<"events"> });
    }
  },
});
